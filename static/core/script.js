...
// (c) 2021-2022 Johnathan Pennington | All rights reserved.
...
topSurface.addEventListener('mousedown', event => {
...
topSurface.addEventListener('mousemove', event => {
...
topSurface.addEventListener('touchstart', event => {
    event.preventDefault();
    touchUpdateControls(event);
});

topSurface.addEventListener('touchmove', event => {
    event.preventDefault();
    touchUpdateControls(event);
});

topSurface.addEventListener('touchend', event => {
    event.preventDefault();
    if (event.touches.length > 0) { touchUpdateControls(event);
    } else { untriggerUpdateControls('touch'); };
});

topSurface.addEventListener('touchcancel', event => {
    event.preventDefault();
    if (event.touches.length > 0) { touchUpdateControls(event);
    } else { untriggerUpdateControls('touch'); };
});
...
function adminAlert() {
    ...
    const appName = 'Core';
    const firstAlertSecs = 25;  // Send first alert after X seconds.
    const alertSpacingFactor = 3;  // Exponential proportion for consecutive alert times.
    const maxSpacingMins = 8;  // A next alert will not be scheduled to be allowed greater than this duration in minutes.
